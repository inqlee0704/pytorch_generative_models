...
class Generator(nn.Module):
    def __init__(self, latent_size=100, in_channels=1, num_class=10):
        super(Generator, self).__init__()
        self.main = nn.Sequential(
            nn.ConvTranspose2d(
                in_channels=latent_size+num_class,
                out_channels=1024,
                kernel_size=4,
                stride=1,
                padding=0,
    ...
    def forward(self, noise, label):
        one_hot_label = nn.functional.one_hot(label,num_classes=10)
        one_hot_label = torch.unsqueeze(torch.unsqueeze(one_hot_label,-1),-1)
        input = torch.cat([noise,one_hot_label],dim=1)
...
class Discriminator(nn.Module):
    def __init__(self, in_channels=1, num_class=10):
        ...
        self.conv_comb = nn.Sequential(
            # 1D conv to reshape
            nn.Conv2d(512+num_class, 512, 1, 1, 0, bias=False),
            nn.BatchNorm2d(512),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(512, 1, 4, 1, 0, bias=False),
            nn.Sigmoid(),
    ...
    def forward(self, img, label):
        ...
        one_hot_label = nn.functional.one_hot(label,num_classes=10)
        one_hot_label = torch.unsqueeze(torch.unsqueeze(one_hot_label,-1),-1)
        combined_out = torch.cat([out,one_hot_label.repeat(1,1,4,4)],dim=1)
        
        out = self.conv_comb(combined_out)
        return out
...
def main():
    ...
    fixed_label = torch.arange(0,10).repeat(7)
    fixed_label = fixed_label[:64]
    iter = 0
    for epoch in range(config.epochs):
        iters = len(dataloader)
        pbar = tqdm(enumerate(dataloader), total=iters)
        for i, (real_img, label) in pbar:
            ...
            gen_img = G(z,label.to(config.device))
            G_loss = criterion(D(gen_img,label.to(config.device)), real_label)
            G_loss.backward()
            optimizer_G.step()
            ...
            D_real_output = D(real_img.to(config.device),label.to(config.device))
            D_real_loss = criterion(D_real_output, real_label)
            D_fake_output = D(gen_img.detach(),label.to(config.device))
        ...
        with torch.no_grad():
            ref_img = G(fixed_noise,fixed_label.to(config.device)).detach().cpu()
